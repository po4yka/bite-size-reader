...
description: Understand and debug the three-writer vector index sync between Postgres summaries/repositories and the Qdrant vector store. Trigger keywords -- vector, Qdrant, embeddings, CocoIndex, reconciler, last_indexed_at, point_ids, semantic search, vector reconcile.
version: 1.0.0
...
Summary and repository embeddings live in two stores that must stay consistent: relational rows in Postgres (`summary_embeddings`, `repository_embeddings`) and vector points in Qdrant. Three cooperating writers keep them aligned.

## The Three Writers

| Writer | Where | When |
| ------ | ----- | ---- |
| **Fast path** | `SummaryEmbeddingGenerator` (summaries), GitHub repo analysis (repos) | Synchronously on summary creation / repo analysis -- the user gets a fresh vector immediately |
| **CocoIndex live updater** | FastAPI process, gated by `RATATOSKR_COCOINDEX_ENABLED=1` | Polls every `RATATOSKR_COCOINDEX_POLL_INTERVAL_SEC` (default 30s) for changed rows; opt-in |
...
The reconciler is the safety net. CocoIndex is optional. The fast path covers steady-state writes. All three converge on the same Qdrant collection using shared deterministic point IDs.

## Deterministic Point IDs

Defined in `app/infrastructure/vector/point_ids.py`. The same summary always maps to the same Qdrant point ID across all three writers, so they don't fight each other.
...
| Qdrant point count != Postgres row count | One writer is erroring silently | Check logs for `cocoindex` or `vector.reconcile` errors |
| Search returns nothing | Collection dimension mismatch after provider switch | Run `backfill_vector_store` |
| CocoIndex eats CPU on FastAPI | `RATATOSKR_COCOINDEX_BATCH_SIZE` too high | Lower it, or set `RATATOSKR_COCOINDEX_ENABLED=0` and rely on the reconciler |
...
- **CocoIndex flow**: `app/infrastructure/cocoindex/flow.py`
...
- **Ops doc**: `docs/cocoindex.md`

## Important Notes

- All three writers MUST go through `point_ids.py` -- never compute Qdrant IDs ad-hoc.
