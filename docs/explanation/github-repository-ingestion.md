...
- Token values never appear in log output. The `logging_utils.py` redaction applies to any field name matching `token|secret|key`.
...
**Key rotation (future work):** the `cryptography` library's `MultiFernet` accepts a list of keys and tries them in order, decrypting with the first that succeeds and re-encrypting with the primary. This enables zero-downtime key rotation. The current implementation uses a single-key `Fernet` instance; a `MultiFernet` migration path is documented in `token_crypto.py` but not yet implemented.
