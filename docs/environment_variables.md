...
| Variable | Default | Description |
| ---------- | --------- | ------------- |
| `MIGRATION_TELEGRAM_RUNTIME_BACKEND` | `python` | Telegram command route-decision backend: `python` (rollback mode) or `rust` (authoritative fail-closed mode). |
...
| `MIGRATION_INTERFACE_EMIT_MATCH_LOGS` | `false` | Deprecated (ignored after fallback decommission) |
| `MIGRATION_INTERFACE_MAX_DIFFS` | `8` | Deprecated (ignored after fallback decommission) |
| `MIGRATION_TELEGRAM_RUNTIME_BACKEND` | `python` | M6 Telegram command route-decision backend (`python` explicit rollback mode, `rust` fail-closed mode) |
