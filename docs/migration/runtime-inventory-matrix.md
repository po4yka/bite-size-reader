...
| Mobile API request execution and background processing | `app/api/main.py`, `app/api/routers/*`, `app/api/background_processor.py`, `app/api/services/*` | `rust/crates/bsr-mobile-api` + `rust/bin/bsr-api` | `python-owned` | `cargo test -p bsr-mobile-api --manifest-path rust/Cargo.toml` + `bash scripts/migration/run_m8_api_runtime_suite.sh` validate the Rust API shell plus Rust-native `auth` / `user` / `system` coverage: canonical OpenAPI/docs/static routes, JWT-vs-WebApp auth precedence, Redis-aware rate limiting, owner-gated system routes, DB dump/range handling, and the full `/v1/auth/*`, `/v1/user/*`, `/v1/system/*` path set. Remaining API groups and background processing are still Python-owned. | Planned: `MIGRATION_API_RUNTIME_BACKEND` |
| Persistence/service orchestration (SQLite access + write paths) | `app/db/*`, `app/infrastructure/persistence/*`, `app/services/*` | `rust/crates/bsr-persistence` | `python-owned` | Processing-critical request/summary/llm/crawl CRUD plus the auth/user/system SQLite subset (users, refresh tokens, client secrets, stats queries, DB info) now run through `bsr-persistence`; broader API/search/sync/service persistence remains Python-owned and still needs snapshot parity coverage | Planned: `MIGRATION_PERSISTENCE_BACKEND` |
