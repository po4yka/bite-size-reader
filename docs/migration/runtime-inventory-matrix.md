...
| Mobile API request execution and background processing | `app/api/main.py`, `app/api/routers/*`, `app/api/background_processor.py`, `app/api/services/*` | `rust/crates/bsr-mobile-api` + `rust/bin/bsr-api` | `python-owned` | `cargo test -p bsr-mobile-api --manifest-path rust/Cargo.toml` + `bash scripts/migration/run_m8_api_runtime_suite.sh` now validate the Rust API shell plus Rust-native `auth` / `user` / `system` and content/submission coverage: canonical OpenAPI/docs/static routes, JWT-vs-WebApp auth precedence, Redis-aware rate limiting, owner-gated system routes, db-dump/range handling, `/v1/summaries/*` + `/v1/articles/*`, `/v1/requests/*`, `/v1/proxy/image`, `/v1/notifications/device`, `/v1/summaries/{summary_id}/audio`, Rust-owned API submit/retry execution, and the focused cross-runtime parity corpus in `python scripts/migration/run_m8_content_domain_parity.py`. Remaining API groups (`search`, `sync`, `collections`, `digest`) and broader background processing are still Python-owned. | Planned: `MIGRATION_API_RUNTIME_BACKEND` |
| Persistence/service orchestration (SQLite access + write paths) | `app/db/*`, `app/infrastructure/persistence/*`, `app/services/*` | `rust/crates/bsr-persistence` | `python-owned` | Processing-critical request/summary/llm/crawl CRUD, the auth/user/system SQLite subset (users, refresh tokens, client secrets, stats queries, DB info), and the content/submission API subset (summary list/detail/content CRUD, request status read-models, notification devices, audio generation metadata) now run through `bsr-persistence`; broader API/search/sync/service persistence remains Python-owned and still needs snapshot parity coverage | Planned: `MIGRATION_PERSISTENCE_BACKEND` |
