...
| URL/forward summarization orchestration (network + pipeline composition) | `app/adapters/content/url_processor.py`, `app/adapters/content/llm_summarizer.py`, `app/adapters/telegram/forward_processor.py` | `rust/crates/bsr-processing-orchestrator` + `rust/crates/bsr-worker` (`bsr-worker`) | `rust-authoritative` | `bash scripts/migration/run_processing_orchestrator_parity_suite.sh` + `bash scripts/migration/run_worker_chunked_parity_suite.sh` + `cargo test -p bsr-persistence -p bsr-processing-orchestrator -p bsr-worker --manifest-path rust/Cargo.toml` + Python bridge routing/NDJSON tests for standard web URL single-pass / multimodal URL / chunked URL / forward-text cutover. Specialized Twitter/YouTube extraction and web-search enrichment are still outside this slice. | `MIGRATION_PROCESSING_ORCHESTRATOR_BACKEND` (`MIGRATION_WORKER_BACKEND` is secondary/test-only when orchestrator mode is Rust) |
| Mobile API request execution and background processing | `app/api/main.py`, `app/api/routers/*`, `app/api/background_processor.py`, `app/api/services/*` | `rust/crates/bsr-mobile-api` + `rust/bin/bsr-api` | `python-owned` | `cargo test -p bsr-mobile-api --manifest-path rust/Cargo.toml` + `bash scripts/migration/run_m8_api_runtime_suite.sh` now validate the Rust API shell plus Rust-native `auth` / `user` / `system`, content/submission, and `sync` / `collections` coverage: canonical OpenAPI/docs/static routes, JWT-vs-WebApp auth precedence, Redis-aware rate limiting, owner-gated system routes, db-dump/range handling, `/v1/summaries/*` + `/v1/articles/*`, `/v1/requests/*`, `/v1/proxy/image`, `/v1/notifications/device`, `/v1/summaries/{summary_id}/audio`, `/v1/sync/*`, `/v1/collections/*`, Rust-owned API submit/retry execution, the focused cross-runtime parity corpus in `python scripts/migration/run_m8_content_domain_parity.py`, and the sync/collections parity corpus in `python scripts/migration/run_m8_sync_collection_parity.py` (with `/v1/collections/tree` covered in direct Rust HTTP tests because the current FastAPI route stack shadows that path with `/{collection_id}` during mounted app execution). Remaining API groups (`search`, `digest`) and broader background processing are still Python-owned. | Planned: `MIGRATION_API_RUNTIME_BACKEND` |
| Persistence/service orchestration (SQLite access + write paths) | `app/db/*`, `app/infrastructure/persistence/*`, `app/services/*` | `rust/crates/bsr-persistence` | `python-owned` | Processing-critical request/summary/llm/crawl CRUD, the auth/user/system SQLite subset (users, refresh tokens, client secrets, stats queries, DB info), the content/submission API subset (summary list/detail/content CRUD, request status read-models, notification devices, audio generation metadata), and the sync/collections subset (collections, collection items/collaborators/invites, sync entity read-models, summary sync-apply writes) now run through `bsr-persistence`; broader API/search/digest/service persistence remains Python-owned and still needs snapshot parity coverage | Planned: `MIGRATION_PERSISTENCE_BACKEND` |
...
3. Persistence crate (`bsr-persistence`) beyond the processing-critical CRUD already cut over in M7.
4. MCP/gRPC interop gateway (`bsr-interop-gateway`) after core runtime cutover.
...
- **Slash-privacy-message command-shape parity hardening (M6-S128):**
  slash-privacy-message fixture `"/\u009Efindonline rust"` is now explicitly
  covered in both `bsr-telegram-runtime` Rust command-route tests and Python
  bridge routing tests to keep non-handled passthrough semantics stable when
...
- **Slash-null command-shape parity hardening (M6-S130):**
  slash-null fixture `"/\u0000findonline rust"` is now explicitly covered in
  both `bsr-telegram-runtime` Rust command-route tests and Python bridge
  routing tests to keep non-handled passthrough semantics stable when
...
- **Slash-start-of-heading command-shape parity hardening (M6-S131):**
  ...
  rather than a command token.
- **Slash-start-of-text command-shape parity hardening (M6-S132):**
  slash-start-of-text fixture `"/\u0002findonline rust"` is now explicitly
  covered in both `bsr-telegram-runtime` Rust command-route tests and Python
  bridge routing tests to keep non-handled passthrough semantics stable when
  slash-prefixed text is followed by a start-of-text control character rather
  than a command token.
- **Slash-end-of-text command-shape parity hardening (M6-S133):**
  slash-end-of-text fixture `"/\u0003findonline rust"` is now explicitly
  covered in both `bsr-telegram-runtime` Rust command-route tests and Python
  bridge routing tests to keep non-handled passthrough semantics stable when
  slash-prefixed text is followed by an end-of-text control character rather
  than a command token.
- **Slash-end-of-transmission command-shape parity hardening (M6-S134):**
  slash-end-of-transmission fixture `"/\u0004findonline rust"` is now
  explicitly covered in both `bsr-telegram-runtime` Rust command-route tests
  and Python bridge routing tests to keep non-handled passthrough semantics
  stable when slash-prefixed text is followed by an end-of-transmission
  control character rather than a command token.
