...
| Mobile API request execution and background processing | `app/api/main.py`, `app/api/routers/*`, `app/api/background_processor.py`, `app/api/services/*` | `rust/crates/bsr-mobile-api` + `rust/bin/bsr-api` | `python-owned` | `cargo test -p bsr-mobile-api --manifest-path rust/Cargo.toml` + `bash scripts/migration/run_m8_api_runtime_suite.sh` validate the Rust API shell foundation: canonical OpenAPI/docs/static routes, `/web` shell, auth/rate-limit middleware scaffolding, and placeholder `/v1/*` route-method parity from `docs/openapi/mobile_api.yaml`. Domain handlers, background processing, and API-domain persistence are still Python-owned. | Planned: `MIGRATION_API_RUNTIME_BACKEND` |
| Persistence/service orchestration (SQLite access + write paths) | `app/db/*`, `app/infrastructure/persistence/*`, `app/services/*` | `rust/crates/bsr-persistence` | `python-owned` | Processing-critical request/summary/llm/crawl CRUD used by URL/forward execution now runs through `bsr-persistence`; broader API/search/sync/service persistence remains Python-owned and still needs snapshot parity coverage | Planned: `MIGRATION_PERSISTENCE_BACKEND` |
