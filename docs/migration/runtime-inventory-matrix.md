...
| URL/forward summarization orchestration (network + pipeline composition) | `app/adapters/content/url_processor.py`, `app/adapters/content/llm_summarizer.py`, `app/adapters/telegram/forward_processor.py` | `rust/crates/bsr-processing-orchestrator` + `rust/crates/bsr-worker` (`bsr-worker`) | `python-owned` | `bash scripts/migration/run_processing_orchestrator_parity_suite.sh` + `bash scripts/migration/run_worker_chunked_parity_suite.sh` + `cargo test -p bsr-worker --manifest-path rust/Cargo.toml` + Python bridge routing tests for single-pass URL / multimodal URL / forward-text / chunked URL worker cutover | `MIGRATION_PROCESSING_ORCHESTRATOR_BACKEND`, `MIGRATION_WORKER_BACKEND` |
