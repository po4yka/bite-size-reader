...
- [ ] #task Introduce SQLAlchemy async engine and session factory #repo/ratatoskr #area/db #status/doing 🔺
...
- [x] `DATABASE_URL` parsed in `app/config/settings.py`; `settings.runtime` exposes a
      typed `DatabaseConfig(dsn, pool_size=8, max_overflow=4)`.
- [x] FastAPI dependency `get_session_for_request` opens a transaction per request,
      commits on success, rolls back on exception, always closes.
- [x] Bot/CLI usage uses the `async with database.session(): …` pattern.
- [x] `with_serialization_retry` retries up to 3× with exponential backoff, only on
      `sqlalchemy.exc.OperationalError` whose pgcode matches `40001` or `40P01`
      (deadlock).
- [x] `app/db/runtime/{operation_executor,rw_lock}.py` and `app/db/_models_*.py` are
...
- SQLAlchemy async factory, session helpers, retry helper, `Base`, and
  `DatabaseConfig.dsn` are in place.
- The live `Ping` session round-trip passed against a throwaway Postgres 16
  container on 2026-05-06; Alembic `migrate()` remains pending O5's async
  Alembic env conversion.
- Remaining `DatabaseSessionManager` imports are deliberately left for the R-phase
  call-site port; `make type` currently fails on missing SQLAlchemy model exports
  until M1/M2 restore the model surface.
