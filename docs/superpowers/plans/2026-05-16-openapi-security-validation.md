# OpenAPI Security Consistency Validation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a CI test that cross-checks FastAPI route security dependencies against the hand-written OpenAPI YAML spec so no protected route can silently appear as public.

**Architecture:** A new test file walks FastAPI route `.dependant` trees to classify routes as code-protected (uses `get_current_user`) or public, then compares against `docs/openapi/mobile_api.yaml` security declarations. Four tests cover: protected→spec mismatch, unrecognized public routes, stale allowlist entries, and missing 403 docs on owner-only paths. CI step expanded to run the new file.

**Tech Stack:** pytest, FastAPI `.dependant` API, PyYAML (already in test deps), existing `test_openapi_sync.py` constants.
...
|------|--------|---------------|
| `tests/api/test_openapi_security.py` | Create | All four security consistency tests |
| `.github/workflows/ci.yml` | Modify line 206 | Include new test file in sync step |

---

### Task 1: Create `tests/api/test_openapi_security.py`

**Files:**
- Create: `tests/api/test_openapi_security.py`
...
def _get_code_protected_routes(app: Any, protected_fn: Any) -> set[tuple[str, str]]:
    ...
    for route in app.routes:
        if not (hasattr(route, "dependant") and hasattr(route, "methods") and hasattr(route, "path")):
            continue
...
```bash
source .venv/bin/activate && pytest tests/api/test_openapi_security.py -v --tb=short
```

Expected: all 4 tests PASS. If any fail, the error message will identify the exact routes and the fix required.

- [ ] **Step 3: Commit the new test file**

```bash
git add tests/api/test_openapi_security.py
git commit -m "test(api): add OpenAPI security consistency validation"
```

---

### Task 2: Update CI to run the new test file

**Files:**
- Modify: `.github/workflows/ci.yml:206`

- [ ] **Step 1: Update the CI test step**

In `.github/workflows/ci.yml`, replace line 206:
...
- [ ] **Step 2: Verify CI step looks correct**

```bash
grep -A 3 "Check OpenAPI spec sync" .github/workflows/ci.yml
```
...
- [ ] **Step 3: Run the full test suite for the OpenAPI tests to confirm no regressions**

```bash
source .venv/bin/activate && pytest tests/api/test_openapi_sync.py tests/api/test_openapi_security.py -v --tb=short
```
...
git commit -m "ci: add OpenAPI security consistency test to sync step"
```
