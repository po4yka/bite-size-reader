...
Use this guide when you already have a running Ratatoskr server and want to access mixed-source aggregation through the external API, packaged CLI, or hosted MCP surface.
...
- the server base URL, for example `https://ratatoskr.example.com`
...
- `ratatoskr login` calls `POST /v1/auth/secret-login`
- the CLI stores the returned access and refresh tokens in `~/.config/ratatoskr/config.toml`
- the CLI refreshes the access token automatically when it is close to expiry
- if refresh later fails because the session was revoked or the refresh token was invalidated, run `ratatoskr login` again with an active secret
...
ratatoskr aggregation list --limit 20
```
...
ratatoskr --json aggregation list --limit 5 | jq '.sessions[] | {id, status, progress}'
```
...
Then connect your MCP client to `https://ratatoskr.example.com/sse` and send:
...
2. Re-open a specific stored run with `get_aggregation_bundle(session_id)` or `ratatoskr://aggregations/{session_id}`
...
- revoked refresh session or logout: run `ratatoskr login` again
