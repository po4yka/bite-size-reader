...
- `bsr-worker`: Rust OpenRouter execution path for single-pass URL, chunked URL, and forwarded-text summaries
...
- `MIGRATION_WORKER_BACKEND=rust` enables the first execution-owned worker slice for single-pass URL, chunked URL, and forward-text summaries.
