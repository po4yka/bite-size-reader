...
- **Post-M5 first implementation slice (M6-S1 scaffold implemented)**
  - Telegram command lifecycle decision scaffold (Rust route decisions,
    ...
    - M6-S94 slash-file-separator command-shape parity hardening
      ...
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S95 slash-group-separator command-shape parity hardening
      (slash-group-separator fixture `"/\u001Dfindonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a group separator control character
      instead of a command token across Rust route decisions + Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S96 slash-record-separator command-shape parity hardening
      (slash-record-separator fixture `"/\u001Efindonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a record separator control character
      instead of a command token across Rust route decisions + Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S97 slash-unit-separator command-shape parity hardening
      (slash-unit-separator fixture `"/\u001Ffindonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a unit separator control character
      instead of a command token across Rust route decisions + Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S98 slash-delete command-shape parity hardening
      (slash-delete fixture `"/\u007Ffindonline rust"` is explicitly covered to
      preserve non-handled passthrough semantics for command-shaped text where
      slash is followed by a delete control character instead of a command
      token across Rust route decisions + Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S99 slash-padding-character command-shape parity hardening
      (slash-padding-character fixture `"/\u0080findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a padding control
      character instead of a command token across Rust route decisions +
      Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S100 slash-high-octet-preset command-shape parity hardening
      (slash-high-octet-preset fixture `"/\u0081findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a high-octet-preset
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S101 slash-break-permitted-here command-shape parity hardening
      (slash-break-permitted-here fixture `"/\u0082findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a break-permitted-here
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S102 slash-no-break-here command-shape parity hardening
      (slash-no-break-here fixture `"/\u0083findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a no-break-here control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S103 slash-index command-shape parity hardening
      (slash-index fixture `"/\u0084findonline rust"` is explicitly covered to
      preserve non-handled passthrough semantics for command-shaped text where
      slash is followed by an index control character instead of a command
      token across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S104 slash-start-of-selected-area command-shape parity hardening
      (slash-start-of-selected-area fixture `"/\u0086findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a start-of-selected-area
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S105 slash-end-of-selected-area command-shape parity hardening
      (slash-end-of-selected-area fixture `"/\u0087findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by an end-of-selected-area
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S106 slash-character-tabulation-set command-shape parity hardening
      (slash-character-tabulation-set fixture `"/\u0088findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a
      character-tabulation-set control character instead of a command token
      across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S107 slash-character-tabulation-with-justification command-shape
      parity hardening (slash-character-tabulation-with-justification fixture
      `"/\u0089findonline rust"` is explicitly covered to preserve non-handled
      passthrough semantics for command-shaped text where slash is followed by
      a character-tabulation-with-justification control character instead of a
      command token across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S108 slash-line-tabulation-set command-shape parity hardening
      (slash-line-tabulation-set fixture `"/\u008Afindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a line-tabulation-set
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S109 slash-partial-line-forward command-shape parity hardening
      (slash-partial-line-forward fixture `"/\u008Bfindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a partial-line-forward
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S110 slash-partial-line-backward command-shape parity hardening
      (slash-partial-line-backward fixture `"/\u008Cfindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a partial-line-backward
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S111 slash-reverse-line-feed command-shape parity hardening
      (slash-reverse-line-feed fixture `"/\u008Dfindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a reverse-line-feed
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S112 slash-single-shift-two command-shape parity hardening
      (slash-single-shift-two fixture `"/\u008Efindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a single-shift-two
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S113 slash-single-shift-three command-shape parity hardening
      (slash-single-shift-three fixture `"/\u008Ffindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a single-shift-three
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S114 slash-device-control-string command-shape parity hardening
      (slash-device-control-string fixture `"/\u0090findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a device-control-string
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S115 slash-private-use-one command-shape parity hardening
      (slash-private-use-one fixture `"/\u0091findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a private-use-one control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S116 slash-private-use-two command-shape parity hardening
      (slash-private-use-two fixture `"/\u0092findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a private-use-two control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S117 slash-set-transmit-state command-shape parity hardening
      (slash-set-transmit-state fixture `"/\u0093findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a set-transmit-state
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S118 slash-cancel-character command-shape parity hardening
      (slash-cancel-character fixture `"/\u0094findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a cancel-character control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S119 slash-message-waiting command-shape parity hardening
      (slash-message-waiting fixture `"/\u0095findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a message-waiting control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S120 slash-start-of-guarded-area command-shape parity hardening
      (slash-start-of-guarded-area fixture `"/\u0096findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a start-of-guarded-area
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S121 slash-end-of-guarded-area command-shape parity hardening
      (slash-end-of-guarded-area fixture `"/\u0097findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by an end-of-guarded-area
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S122 slash-start-of-string command-shape parity hardening
      (slash-start-of-string fixture `"/\u0098findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a start-of-string control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S123 slash-single-graphic-character-introducer command-shape parity
      hardening (slash-single-graphic-character-introducer fixture
      `"/\u0099findonline rust"` is explicitly covered to preserve non-handled
      passthrough semantics for command-shaped text where slash is followed by
      a single-graphic-character-introducer control character instead of a
      command token across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S124 slash-single-character-introducer command-shape parity hardening
      (slash-single-character-introducer fixture `"/\u009Afindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a
      single-character-introducer control character instead of a command token
      across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S125 slash-control-sequence-introducer command-shape parity hardening
      (slash-control-sequence-introducer fixture `"/\u009Bfindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by a
      control-sequence-introducer control character instead of a command token
      across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S126 slash-string-terminator command-shape parity hardening
      (slash-string-terminator fixture `"/\u009Cfindonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a string-terminator control character
      instead of a command token across Rust route decisions and Python bridge
      tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S127 slash-operating-system-command command-shape parity hardening
      (slash-operating-system-command fixture `"/\u009Dfindonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by an
      operating-system-command control character instead of a command token
      across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S128 slash-privacy-message command-shape parity hardening
      (slash-privacy-message fixture `"/\u009Efindonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a privacy-message control character
      instead of a command token across Rust route decisions and Python bridge
      tests): `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S129 slash-application-program-command command-shape parity hardening
      (slash-application-program-command fixture
      `"/\u009Ffindonline rust"` is explicitly covered to preserve
      non-handled passthrough semantics for command-shaped text where slash is
      followed by an application-program-command control character instead of a
      command token across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S130 slash-null command-shape parity hardening
      ...
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S131 slash-start-of-heading command-shape parity hardening
      (slash-start-of-heading fixture `"/\u0001findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a start-of-heading control character
      instead of a command token across Rust route decisions and Python bridge
      tests): `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S132 slash-start-of-text command-shape parity hardening
      (slash-start-of-text fixture `"/\u0002findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by a start-of-text control character
      instead of a command token across Rust route decisions and Python bridge
      tests): `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S133 slash-end-of-text command-shape parity hardening
      (slash-end-of-text fixture `"/\u0003findonline rust"` is explicitly
      covered to preserve non-handled passthrough semantics for command-shaped
      text where slash is followed by an end-of-text control character instead
      of a command token across Rust route decisions and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
      `docs/migration/runtime-inventory-matrix.md`
    - M6-S134 slash-end-of-transmission command-shape parity hardening
      (slash-end-of-transmission fixture `"/\u0004findonline rust"` is
      explicitly covered to preserve non-handled passthrough semantics for
      command-shaped text where slash is followed by an end-of-transmission
      control character instead of a command token across Rust route decisions
      and Python bridge tests):
      `rust/crates/bsr-telegram-runtime/src/lib.rs`,
      `tests/test_message_router_interface_routing.py`,
