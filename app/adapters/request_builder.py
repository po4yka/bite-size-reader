...
from app.adapters.llm_models import ChatRequest
...
class RequestBuilder:
    ...
    def build_request_body(
        ...
        if response_format and self._enable_structured_outputs:
            built_rf = self._build_response_format(
                response_format, self._structured_output_mode)
            if built_rf:
    ...
    def should_apply_compression(
        ...
        total_content_length = sum(len(msg.get("content", ""))
                                   for msg in messages)
