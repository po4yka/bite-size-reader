...
from app.adapters.error_handler import ErrorHandler
from app.adapters.llm_models import ChatRequest, LLMCallResult
from app.adapters.model_capabilities import ModelCapabilities
from app.adapters.payload_logger import PayloadLogger
from app.adapters.request_builder import RequestBuilder
from app.adapters.response_processor import ResponseProcessor
    ...
    async def chat(
        ...
        *,
        ...
        for model in models_to_try:
            ...
            requested_rf = response_format if isinstance(
            ...
            for attempt in range(self.error_handler._max_retries + 1):
                ...
                if should_compress and transform_type:
                    ...
                    self.payload_logger.log_compression_applied(
                        total_length, 200000, model)
                ...
                try:
                    self.payload_logger.log_request(
                        model, attempt, len(
                            sanitized_messages), rf_included, rf_mode_current
                    )
                    ...
                    self.payload_logger.log_response(
                        resp.status_code, latency, last_model_reported)
                    ...
                    error_message = self.response_processor.get_error_message(
                        status_code, data)
