...
class StatusUpdateSynchronizer:
        ...
        *,
        ...
        try:
            ...
            for sync_record in synced_items:
                delta = await self._sync_single_record(
                    sync_record=sync_record,
                    repository=repository,
                    client=client,
                    karakeep_by_id=karakeep_by_id,
        ...
        logger.info(
            ...
            extra={
                "correlation_id": correlation_id,
                "bsr_to_karakeep": stats["bsr_to_karakeep_updated"],
                "karakeep_to_bsr": stats["karakeep_to_bsr_updated"],
                "tags_added": stats["tags_added"],
                "tags_removed": stats["tags_removed"],
                "favourites_updated": stats["favourites_updated"],
                "errors": len(errors),
            },
        )
    ...
    async def _sync_single_record(
        ...
        *,
        ...
        repository: KarakeepSyncRepository,
        client: KarakeepClientProtocol,
        karakeep_by_id: dict[str, KarakeepBookmark],
        correlation_id: str,
        errors: list[str],
    ) -> dict[str, int]:
        delta = {
        ...
        try:
            ...
            bsr_is_source = self._is_bsr_source(sync_record, summary_data, bookmark)
            if bsr_is_source:
                await self._sync_bsr_to_karakeep(
                    delta=delta,
                    summary_data=summary_data,
                    bookmark=bookmark,
                    bsr_read=bsr_read,
                    bsr_fav=bsr_fav,
                    kk_has_read_tag=kk_has_read_tag,
                    kk_fav=kk_fav,
                    sync_id=sync_id,
                    client=client,
                    repository=repository,
                ...
                delta["karakeep_to_bsr_updated"] += 1
                logger.debug(
    ...
    def _is_bsr_source(
        ...
        if summary_updated_at and bookmark_updated_at:
            if summary_updated_at > bookmark_updated_at:
                return True
            if bookmark_updated_at > summary_updated_at:
                return False
            ...
            if bookmark_updated_at > karakeep_modified_at:
                return False
        return bsr_is_source

    async def _sync_bsr_to_karakeep(
        self,
        *,
        delta: dict[str, int],
        summary_data: dict,
        ...
        if tags_to_add:
            ...
            if success:
                delta["tags_added"] += len(tags_to_add)
                needs_update = True
                if isinstance(updated, KarakeepBookmark) and updated.modified_at:
                    last_karakeep_modified_at = updated.modified_at
            else:
                errors.append(f"Failed to attach tags for bookmark {bookmark.id}: {error}")
                logger.warning(
        ...
        for tag_id in tags_to_remove:
            ...
            if success:
                delta["tags_removed"] += 1
                needs_update = True
                last_karakeep_modified_at = datetime.now(UTC)
            else:
                errors.append(f"Failed to detach tag for bookmark {bookmark.id}: {error}")
                logger.warning(
        ...
        delta["bsr_to_karakeep_updated"] += 1
        logger.debug(
