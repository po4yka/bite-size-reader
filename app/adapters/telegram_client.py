    ...
    async def _setup_bot_commands(self) -> None:
        ...
        try:
            ...
            commands_en = [
                ...
                BotCommand("summarize_all",
                           "Summarize multiple URLs from one message"),
                BotCommand("summarize_forward",
                           "Summarize the next forwarded channel post"),
            ]
            commands_ru = [
                BotCommand("start", "Приветствие и инструкция"),
                BotCommand("help", "Показать помощь и инструкцию"),
                BotCommand(
                    "summarize", "Суммировать ссылку (или пришлите позже)"),
                BotCommand("summarize_all",
                           "Суммировать несколько ссылок из сообщения"),
                BotCommand("summarize_forward",
                           "Суммировать следующий пересланный пост"),
            ]
            try:
                client_any: Any = self.client
                ...
                logger.info(
                    "bot_commands_set",
                    extra={"count_en": len(commands_en),
                           "count_ru": len(commands_ru)},
                )
            except Exception as e:  # noqa: BLE001
                logger.warning("bot_commands_set_failed",
                               extra={"error": str(e)})
        except Exception:
            return
