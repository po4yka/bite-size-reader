...
class BatchSession(Base):
    ...
    items: Mapped[list[Any]] = relationship(
        "BatchSessionItem", back_populates="batch_session", cascade="all, delete-orphan"
    )
...
class BatchSessionItem(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "ix_batch_session_items_session_request", "batch_session_id", "request_id", unique=True
        ),
        Index("ix_batch_session_items_is_series_part", "is_series_part"),
    )

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    batch_session_id: Mapped[int] = mapped_column(
        ForeignKey("batch_sessions.id", ondelete="CASCADE"), nullable=False
    )
