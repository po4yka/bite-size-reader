...
def test_digest_facade_delegates_sync_methods() -> None:
    ...
    assert facade.list_channel_posts(10, "@chan", limit=5, offset=2) == {
        "posts": [],
        "total": 0,
    }
...
def test_digest_facade_trigger_digest_enqueues_job() -> None:
    service = MagicMock()
    ...
    service.enqueue_digest_trigger.assert_called_once_with(user_id=77, correlation_id="cid-123")


def test_digest_facade_trigger_channel_digest_uses_normalized_channel() -> None:
    service = MagicMock()
    ...
    service.enqueue_channel_digest_trigger.assert_called_once_with(
        user_id=88,
