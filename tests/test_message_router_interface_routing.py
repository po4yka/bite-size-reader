...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_start_of_selected_area_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_end_of_selected_area_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_character_tabulation_set_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_character_tabulation_with_justification_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_line_tabulation_set_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_partial_line_forward_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_partial_line_backward_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_single_shift_two_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_single_shift_three_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_device_control_string_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_private_use_one_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_private_use_two_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_cancel_character_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_message_waiting_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_start_of_guarded_area_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_end_of_guarded_area_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_start_of_string_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_single_graphic_character_introducer_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_single_character_introducer_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_control_sequence_introducer_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_string_terminator_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_operating_system_command_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_privacy_message_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_application_program_command_command_like_text() -> (
    None
):
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_null_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_start_of_heading_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_start_of_text_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_end_of_text_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
...
@pytest.mark.asyncio
async def test_route_command_message_ignores_slash_end_of_transmission_command_like_text() -> None:
    router = _Router()
    router.telegram_runtime_runner.resolve_command_route = AsyncMock(
        return_value=TelegramRuntimeCommandDecision(command=None, handled=False)
    )
