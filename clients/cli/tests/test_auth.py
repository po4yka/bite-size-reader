...
import pytest
from bsr_cli.auth import ensure_authenticated, refresh_if_needed
...
class TestAuth:
    ...
    def test_refresh_when_token_near_expiry(self):
        ...
        with (
            patch("bsr_cli.auth.httpx.post", return_value=mock_resp),
            patch("bsr_cli.auth.save_config"),
        ):
            result = refresh_if_needed(config)
    ...
    def test_ensure_authenticated_raises_without_token(self):
        ...
        config = BSRConfig(server_url="https://test.com")
