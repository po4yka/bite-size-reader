...
from bsr_cli.config import BSRConfig, get_config_dir, load_config, save_config


class TestConfig:
    def test_default_config(self, tmp_path):
        """Empty config file yields defaults."""
        with patch("bsr_cli.config.get_config_dir", return_value=tmp_path):
            cfg = load_config()
    ...
    def test_save_and_load_roundtrip(self, tmp_path):
        ...
        with patch("bsr_cli.config.get_config_dir", return_value=tmp_path):
            cfg = BSRConfig(
                server_url="https://bsr.example.com",
                client_id="test-cli",
                user_id=123,
            ...
            save_config(cfg)
            loaded = load_config()
            assert loaded.server_url == "https://bsr.example.com"
    ...
    def test_file_permissions(self, tmp_path):
        ...
        with patch("bsr_cli.config.get_config_dir", return_value=tmp_path):
            save_config(BSRConfig(server_url="https://test.com"))
    ...
    def test_xdg_config_home(self, tmp_path):
        ...
        with patch.dict(os.environ, {"XDG_CONFIG_HOME": str(tmp_path)}):
            assert get_config_dir() == tmp_path / "bsr"
